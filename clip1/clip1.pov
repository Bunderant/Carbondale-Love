/* CS 484 S11 - Final Lab - Mark Agne and Le Gao
   ...
   Inchworm moves from left to right across the screen.   
*/ 
...
#declare num_loops = 12.0; 
#declare startingPos_x = -27;
...
#declare CamLook = <0,15,0>; // Camera's Look_at 
#declare CamLoc = <0,10,-35>; //where the camera's location is
#declare cam_z = 1.0; //the amount of camera zoom
#declare back_dist = 200; // how far away the background image is
...
box { <0,0,0> <1,1,0.1>
      pigment { image_map { jpeg "clipImage1.jpg" 
                map_type 0 
                interpolate 2 } }
      finish { ambient 0.5 }
      translate <-0.5,-0.5,0>
...
light_source { <5,10, 30> color Gray60 }
light_source { <15,50,5> color White }  
...
//#declare x_max = 2*(sqrt(((num_segments-.4)*(1.3*0.04))/pow(5*0.04,2))+(bodySegmentDist*0.65);

#declare x_max = (bodySegmentDist*num_segments);
#declare worm_displacement = x_max*(anim_clock+current_loop);

object { worm
   translate<startingPos_x+worm_displacement,bodyScale,0>
}